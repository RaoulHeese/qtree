...
class QTreeCircuitManager(object):
    ...
    def gateop_mct_default(circ, q1, q2, qc_list):
        ...
        assert len(qc_list) > 0 and q1 != q2 and q1 not in qc_list and q2 not in qc_list
        circ.mct(qc_list + [q1],
                 q2)  # controls, target; see: https://qiskit.org/documentation/locale/ja/api/qiskit.aqua.circuits.gates.mct.html
    ...
    def _to_data_probabilities(self, x):
        ...
        for key in product('01', repeat=N):
            ...
            p = x[np.logical_and.reduce([x[:, index] == value for index, value in enumerate(values)])].shape[0] / \
                x.shape[0]
            key = key[::-1]  # correct qiskit ordering
    ...
    def build_circuit(self, G, Nx, Ny, init, measure_y=True, measure_all_x=False, remove_inactive_qubits=False,
        ...
        if init is not None:
            if type(init) is tuple:
                ...
                self.lg.debug(
                    "use psi_mode as array (warning: no correction of active qubit ordering etc. is performed!)")
                psi0 = np.array(init)
    ...
    def run_circuits(self, circ_list, quantum_backend):
        ...
        circ_list = transpile(circ_list, backend=quantum_backend)
        result = quantum_backend.run(circ_list).result()

        counts_list = []
        for circ_idx, circ in enumerate(circ_list):
